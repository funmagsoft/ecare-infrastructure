...
> **Note**: This is part of the [ecare-infrastructure monorepo](../../README.md). All paths in this documentation are relative to the `workload` directory unless otherwise specified.
...
- Pulls IDs of ACR, AKS, KV/Storage/SB from `platform` remote state
...
**Important:** This component creates Service Principals with a different naming pattern than the bootstrap Service Principals in `foundation`:
...
- **Bootstrap Service Principals** (`foundation` component): `sp-gha-{project_name}-infra-{environment}`
  - Example: `sp-gha-ecare-infra-dev`
  - **Has `-infra-` suffix** - these are for infrastructure management (Terraform operations)
  - Used by Terraform repositories (`foundation`, `platform`, `workload`)
...
- Remote states (accessed by the `environment` module):
  - `foundation/terraform.tfstate` - Provides Resource Group and location
  - `platform/terraform.tfstate` - Provides IDs of KV/Storage/SB and AKS configuration
...
- RBAC scopes rely on outputs from `platform`. Keep platform deployed and outputs available for each env.
