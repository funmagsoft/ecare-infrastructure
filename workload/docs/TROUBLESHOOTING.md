...
This document provides solutions to common issues encountered when working with the `workload` repository.
...
Error: Failed to get outputs from remote state: platform
```

**Causes:**

1. `platform` not deployed yet
2. Wrong remote state backend configuration
3. Authentication issues

**Solution:**

**Step 1:** Verify `platform` is deployed:
...
If no outputs, deploy `platform` first.

**Step 2:** Verify backend configuration in `workload`:
...
terraform {
  backend "azurerm" {
    ...
    key                  = "workload/terraform.tfstate"  # ← Correct path
  }
}
```
...
data "terraform_remote_state" "platform" {
  ...
  config = {
    ...
    storage_account_name = "tfstatefmsecaredev"
    container_name       = "tfstate"
    key                  = "platform/terraform.tfstate"  # ← Correct
  }
}
```
...
```bash
# Get Terraform Service Principal (from foundation)
...
- `platform` (if it creates namespaces)
