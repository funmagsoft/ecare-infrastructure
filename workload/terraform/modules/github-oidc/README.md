...
**For Terraform state authentication**, see the [`foundation` repository](https://github.com/hycom/foundation) bootstrap module, which creates Service Principals and FIC for Terraform repositories (`foundation`, `platform`, `workload`) to manage infrastructure via Terraform.
...
- **Platform Module** (`platform`): Retrieves ACR and AKS IDs from remote state
...
From Phase 1 (foundation) and Phase 2 (platform):

- Resource Group must exist: `rg-{project_name}-{environment}`
- Azure Container Registry (ACR) must exist (created by platform)
- Azure Kubernetes Service (AKS) cluster must exist (created by platform)
