...
The `foundation` component provides the foundational infrastructure layer for the ecare project. It establishes the core networking infrastructure and authentication mechanisms required for all subsequent infrastructure deployments.
...
- **Federated Identity Credentials (FIC)**: OIDC authentication for Terraform repositories
  - `foundation`: This component
  - `platform`: Platform infrastructure (AKS, ACR, databases)
  - `workload`: Workload identities for services
...
1. **`platform`**: Uses VNet and subnets (via remote state)
   - AKS subnet for Kubernetes nodes
   - Data subnet for database private endpoints
2. **`workload`**: Uses Resource Groups and remote state
