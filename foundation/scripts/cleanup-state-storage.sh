#!/usr/bin/env bash
# =============================================================================
# Script: cleanup-state-storage.sh
# Component: foundation
# Purpose: Delete Terraform state Storage Accounts for selected environments.
# =============================================================================
# Usage:
#   ./cleanup-state-storage.sh [--dry-run|--execute] [--env <env>] [--all-envs] --confirm "DELETE phase0 <project>" [-h|--help]
# =============================================================================
...
usage() {
  cat <<'EOF'
Usage: ./cleanup-state-storage.sh [--dry-run|--execute] [--env <env>] [--all-envs] --confirm "DELETE phase0 <project>" [-h|--help]

Delete Terraform state Storage Accounts (and their containers) for selected environments.
...
Notes:
  ...
  - This script deletes the Storage Account; container cleanup is implicit.
...
log_info "=== Deleting Terraform State Storage Accounts ==="
log_dry_run
...
for ENV in "${TARGET_ENVS[@]}"; do
  ...
  log_info "--- Deleting Storage Account ${SA_NAME} (${ENV}) ---"

  if ! az_call storage account show --name "$SA_NAME" --resource-group "$RG_NAME" --output none 2>/dev/null; then
    log_warning "Storage Account does not exist (skipping)."
    echo ""
    continue
  fi
  ...
  if az_exec storage account delete \
    ...
    log_error "Failed to delete Storage Account"
    ERRORS=$((ERRORS + 1))
  fi
...
log_info "Storage Accounts deleted: ${DELETED}/${#TARGET_ENVS[@]}"
