...
This document defines the naming conventions used in the `infra-identity` repository for Azure AD identities, Kubernetes resources, and related components.
...
- Terraform SP: `sp-gha-ecare-infra-dev` (created by `infra-foundation`)
...
- `infra-platform` (if responsible for cluster setup)
