...
Before deploying `infra-identity`, ensure the following are deployed:

1. **infra-foundation** (Phase 1):
   - ✓ Resource Groups (`rg-ecare-{env}`)
   - ✓ Virtual Network and subnets
   - ✓ Terraform state Storage Account (`tfstatefmsecaredev`)

2. **infra-platform** (Phase 2):
...
cd /path/to/infra-foundation/terraform/environments/dev
terraform output
