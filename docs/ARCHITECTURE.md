...
- **Terraform SP**: `sp-gha-ecare-infra-{env}` (created by `infra-foundation`)
  - Purpose: Manage infrastructure via Terraform
  - Repos: `infra-foundation`, `infra-platform`, `infra-identity`
...
- Remote state dependency: Kubernetes provider needs AKS config from `infra-platform` remote state
...
1. Prerequisites deployed:
   - infra-foundation (VNet, RG, Storage)
   - infra-platform (AKS, ACR, Key Vault, etc.)
        ↓
